...
def  user_Signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if not userExisttt:
            ...
            query = UserSignUp(u_fname= user_Fname , u_lname = user_Lname , u_email = user_Email, u_password = user_password  )
            query.save()
            
            message = {
                    'msg' : "userSignUpSuccessfull"
            }
...
def user_addInCart_view(request):
    ...
    if request.method == "POST":
        ...
        if  userId_Exist:
            ...
            productId_Exist  = Cart.objects.filter( userId_id =  userId ,  productId = productIdd   ).exists()
            ...
            if not productId_Exist:
                ...
                query = Cart(userId_id = userId  , productId_id = productIdd , qty = 1, date = datetime.now() )
                query.save()
...
@api_view(['GET'])
def user_fetchCartByUserId_view(request, pk):
    ...
    if request.method == "GET":
        ...
        userId_Exist = Cart.objects.filter(userId_id =  pk  ).exists()
...
@api_view(['PUT'])
def user_updateCartQtyByProductID_view(request, pk1 , pk2):
    ...
    if request.method == "PUT":
        ...
        productId_Exist = Cart.objects.filter(id =  pk1  ).exists()
        ...
        if  productId_Exist:
            ...
            if pk2 == "incremented":
                ...
                queryGet = Cart.objects.get(id = pk1)
...
@api_view(['DELETE'])
def user_deleteCartItem_view(request, pk1, pk2 ):
    ...
    if request.method == "DELETE":
        ...
        user_Exist = Cart.objects.filter( userId_id  =  pk1  ).exists()

        # print("userId_Exist", userId_Exist)

        if  user_Exist:

            item_Exist = Cart.objects.filter( id = pk2  ).exists()

            if item_Exist:

                query = Cart.objects.get( id = pk2  )
                query.delete()
...
@api_view(['POST', 'GET'])
def  user_address_view(request, pk):
    ...
    if request.method == "GET":
        ...
        getQuery = Address.objects.filter(userId_id = pk).values()
...
def user_PlaceOrder_view(request):
    ...
    if request.method == "POST":
        ...
        query = PlaceOrder( userId_id  = userId  , addressId_id  = addressId, productsArray = cartIds , 
                           cartAmount = (checkoutAmount) , paymentId = 0 , paymentMode = "COD" , trackingNo = randomTrackNo,
                            status = 0, date = datetime.today()  )
